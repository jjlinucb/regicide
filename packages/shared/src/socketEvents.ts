import type { Card, ClientGameState, GameAction, Rank } from './game/types.js';
...
export interface ClientToServerEvents {
  ...
  /** Mission 8's reward, John's ruling (see legacy/party.ts's MissionReward.chanterStickerChoice): permanently gives the named existing party card a bonus Chanter sticker — validated server-side against chanterStickerEligible, not trusted from the client. */
  'legacy:chooseChanterSticker': (payload: { code: string; cardId: string }, cb: (res: { ok: true } | { ok: false; error: string }) => void) => void;
  /**
   * Mission 9's reward, John's ruling (see legacy/party.ts's MissionReward.mageStickerRankChoice): the player picks a RANK (4 or 8) and the
   * SERVER draws a random eligible card of that rank to carry the bonus Mage sticker. A rank, never a card id — the client cannot name the
   * recipient. The rank is re-validated server-side against mageStickerRankOptions, not trusted from the client. The success callback names
   * the card the draw actually landed on (`awardedCardId`) so the picker can show the player who got it instead of the panel just vanishing.
   */
  'legacy:chooseMageStickerRank': (
    payload: { code: string; rank: Rank },
    cb: (res: { ok: true; awardedCardId: string } | { ok: false; error: string }) => void,
  ) => void;
